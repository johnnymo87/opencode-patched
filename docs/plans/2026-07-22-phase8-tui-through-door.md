...
- **D6 — NEW, needs a call (fable HIGH-1): subscription set = attach-time sid, or = active session?** The set is currently pinned at mount. If the user **switches sessions** in-TUI or **creates a new session** in-TUI, that session's events aren't in the set → live-dead view / wedged mid-turn prompt (the exact bug, one dialog away). Also the **sid-less `opencode` root TUI** has no stream through the door (`/global/event`→410, bare `/event`→400). Options: (a) define the set as **f(active route sessionID)** with reconnect-on-change (the D2 reconcile machinery gives this nearly for free — more robust, matches "everything through the door, no exceptions"); (b) explicitly **scope Phase 8/9 to attach-with-`--session` TUIs** and guard/disable the switcher + new-session + sid-less paths (smaller, but a documented limitation). **Leaning (a)** per the no-fallbacks stance.
